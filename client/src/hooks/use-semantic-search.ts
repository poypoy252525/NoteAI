...
export const useSemanticSearch = (): UseSemanticSearchReturn => {
  ...
  const searchNotes = useCallback(
    async (query: string, limit: number = 10, threshold: number = 0.7) => {
      if (!query.trim()) {
        setResults([]);
        return;
      }
  ...
  const findSimilarNotes = useCallback(
    async (noteId: string, limit: number = 5) => {
      try {
        setLoading(true);
        setError(null);
        const similarNotes = await semanticSearchService.findSimilarNotes(
          noteId,
          limit
        );
        ...
        setError(
          err instanceof Error ? err.message : "Failed to find similar notes"
        );
        setResults([]);
      } finally {
        setLoading(false);
